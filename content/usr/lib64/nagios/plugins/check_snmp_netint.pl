...
for (my $i=0;$i < $num_int; $i++) { 
  ...
  if (defined($descr[$i]) && (defined($o_perf) || defined($o_perfs) || defined($o_perfr) || defined($o_perfp) || defined($o_checkperf))) {
	...
	if ($usable_data==0) {
	    ...
	    # WL: have to be more careful to avoid undefined errors in case
	    #     when its first time and no previous performance data exists
	    $perf_out .= sprintf("%.0f",$checkperf_out[0] * 800 / $portspeed[$i]) ."%;" if defined($checkperf_out[0]) && defined($portspeed[$i]) && $portspeed[$i]!=0;
	    $perf_out .= ($o_warn[0]!=0) ? $o_warn[0] . ";" : ";";
	    $perf_out .= ($o_crit[0]!=0) ? $o_crit[0] . ";" : ";"; 
	    $perf_out .= "0;100 ";
	    $perf_out .= " ".perf_name($descr[$i],"out_prct")."=";
	    # WL: see above about being careful and avoiding undefined errors
	    $perf_out .= sprintf("%.0f",$checkperf_out[1] * 800 / $portspeed[$i]) ."%;" if defined($checkperf_out[1]) && defined($portspeed[$i]) && $portspeed[$i]!=0;
