				...
				case 3{	# UPS is running on battery
					if (defined $critical_threshold && $ups_runtime_mins < $critical_threshold){
						...
						exit $CRITICAL;
					        ...
					        print "WARNING: On battery power (Runtime remaining: $ups_runtime_mins minutes)$perf_data\n";
						exit $WARNING;
					}else{
					        print "OK: On battery power (Runtime remaining: $ups_runtime_mins minutes)$perf_data\n";
						exit $OKAY; 
					}
				}
