...
my %CiscoEnvMonNagios = (1,1 ,2,3 ,3,3 ,4,3 ,5,0, 6,3); # Nagios states returned for CIsco states (coded see @Nagios_state).
			...
			if ($key =~ /$cisco_chassis_card_state/) {
				...
				if ($tmp_status == 0) {
					...
					if (!defined($result_t)) { $card_output.="Invalid card(UNKNOWN)";}
                                                ...
                                                if (index($$result_t{$cisco_chassis_card_descr.".".$index}, "SVC-APP-PROC") == -1) {
							...
							# Hack to skip daughter card checks (SVC-APP-PROC) that don't work with this script yet.  -DAJ 9-24-2015
							$num_cards_ok++;
							$final_status = 0;
						}
					}
					...
					if (defined($result_t)) {
						if ($module_slot_present == 1) {
							$modules_output.= "(" .$$result_t{$cisco_module_descr.".".$index} ."): "; 
                                                        ...
                                                        if (index($$result_t{$cisco_module_descr.".".$index}, "SVC-APP-PROC") == -1) {
                                                                if ($modules_output ne "") {$modules_output.=", ";}
                                                                $modules_output.= "Module (" .$$result_t{$cisco_module_descr.".".$index} ."): ";
                                                        } else {
                                                                # Hack to skip daughter card checks (SVC-APP-PROC) that don't work with this script yet.  -DAJ 9-24-2015
                                                                $skip_daughter_card = 1;
                                                                $tmp_status = 0;
					                        $num_modules_ok++;
                                                                $$resultat_m{$key} = 1;
                                                        }
						}
                                                if ($skip_daughter_card == 0) {
